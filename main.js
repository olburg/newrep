    ...
    addDeleteListener() {
        ...
        paragraphs.forEach( p => {
            p.addEventListener('click', event => {
                ...
                let [,id] = event.target.id.split('_'); // ['person', '1234567890']
                const idx = this.persons.findIndex(person => person.id.toString() === id);
                if ( idx === -1 ) {
                    return;
                }
...
// 2. Создаем новый объект и кладем его в массив
document.querySelector('form').addEventListener('submit', event => {
    event.preventDefault();
    const newPerson = new Person( // Создание объекта
        document.querySelector('#fullName').value,
        document.querySelector('#age').value
    );
    document.querySelector('#fullName').value = '';
    document.querySelector('#age').value = '';
    persons.addPersons( newPerson );  // В массив
})
